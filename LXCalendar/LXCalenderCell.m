...
-(void)setModel:(LXCalendarDayModel *)model{
    ...
    if (model.isNextMonth || model.isLastMonth || model.isCurrentMonth==NO) {
...
//                self.label.textColor = model.nextMonthTitleColor? model.nextMonthTitleColor:[UIColor colorWithWhite:0.85 alpha:1.0];
                self.label.textColor = model.nextMonthTitleColor? model.nextMonthTitleColor:[UIColor hexStringToColor:@"bbbbbb"];
            }
            
            if (model.isLastMonth) {
                self.label.textColor = model.lastMonthTitleColor? model.lastMonthTitleColor:[UIColor hexStringToColor:@"bbbbbb"];
            }
...
//            [self.label setFTCornerdious:self.label.lx_width/2];
            ...
            self.label.backgroundColor = model.selectBackColor?model.selectBackColor:[UIColor hexStringToColor:@"31AEEC"];
...
//            if (model.isToday) {
//                self.label.textColor = [UIColor hexStringToColor:@"31AEEC"];
//                self.label.layer.borderWidth = 1;
//                self.label.layer.borderColor = [UIColor hexStringToColor:@"31AEEC"].CGColor;
//            }
    if (model.isDefaule==YES) {
        self.label.textColor = [UIColor whiteColor];
        self.label.backgroundColor = model.selectBackColor?model.selectBackColor:[UIColor hexStringToColor:@"31AEEC"];
    }
